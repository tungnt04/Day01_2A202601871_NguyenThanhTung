...
def call_openai(
    ...
    response = client.chat.completions.create(
        ...
        top_p=top_p,
        max_completion_tokens=max_tokens,   # hoặc max_tokens nếu SDK yêu cầu
    )
...
def chat_with_system_prompt(
    ...
    response = client.chat.completions.create(
        ...
        temperature=temperature,
        max_completion_tokens=max_tokens,   # hoặc max_tokens tùy phiên bản SDK
    )
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        # Fallback khi không tải được encoding hoặc model không hỗ trợ
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Chỉ giữ 3 lượt hội thoại gần nhất (6 message)
        history = history[-6:]
...
def run_assistant(
    ...
    while True:
        ...
        # Chỉ giữ 3 lượt hội thoại gần nhất
        history = history[-6:]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for item in results:
        rows.append(
            f"{shorten(item['prompt']):<42}"
            f"{shorten(item['gpt4o_response']):<42}"
            f"{shorten(item['mini_response']):<42}"
            f"{item['gpt4o_latency']:<18.3f}"
            f"{item['mini_latency']:<18.3f}"
        )
